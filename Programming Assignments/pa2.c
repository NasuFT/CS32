...
BigInt* multiply(BigInt** a, BigInt** b) {
    ...
    while(current_b != NULL) {
        int carry_over = 0;
        BigInt* result_multiplier = NULL;
        while(current_a != NULL) {
            int result = current_a->digit * current_b->digit + carry_over;
            int digit = result % 10;
            carry_over = result / 10;

            add_left(&result_multiplier, digit);

            current_a = current_a->prev;
        }

        if(carry_over > 0) add_left(&result_multiplier, carry_over);
        for(int i = 0; i < b_multiplier; i++) {
            add_right(&result_multiplier, 0);
        }

        result_num = add(&result_num, &result_multiplier);
        current_a = get_tail(a);
...
int main() {
    ...
    BigInt* result = multiply(&num_a, &num_b);
